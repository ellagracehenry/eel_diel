geometric_submodel <- function(data) {
  ...
  avg_hide <- mean(df_run_length_hide$run_length)
  avg_emerge <- mean(df_run_length_emerge$run_length)
  
  switch_to_hide <- 1/avg_emerge
  switch_to_emerge <- 1/avg_hide
  ...
  for(t in 1:(T-1)) {
    for (i in 1:N){
      if (state_record[i,t] == 1) {
        if (rbinom(1,1,switch_to_hide) == 1) {
          state_record[i,t+1] <- 0
        } else {
          state_record[i,t+1] <- 1
        }
      } else {
        if (rbinom(1,1,switch_to_emerge) == 1) {
          state_record[i,t+1] <- 1
        } else {
          state_record[i,t+1] <- 0
        }
